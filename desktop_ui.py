...
class BookListFrame(wx.Frame):
    def __init__(self, repo_path):
        ...
        self.list = wx.ListCtrl(self, -1, style=wx.LC_REPORT|wx.LC_EDIT_LABELS)
    ...
    def SaveDispname(self, event):
        entered = event.GetLabel()
        if not event.IsEditCancelled() and entered:
            controls.update_dispname(self.idx2name[event.GetIndex()], entered)
