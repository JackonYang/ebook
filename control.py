...
from validate import is_deleted, is_ignore, validate_ext
...
class FlatFile:
    ...
    def add_path(self, src_path, ext='*.pdf'):
        ...
        for rel_path in os.listdir(src_path):
            ...
            if is_ignore(rel_path):
                op_log.debug('ignore %s' % rel_path)
            elif os.path.isfile(abs_path):  # file 
                if validate_ext(abs_path, ext):  # extension validate
                    metainfo = self._build_meta(abs_path)
                    if is_deleted(metainfo.file_id):
                        op_log.debug('ignore deleted file: %s' % abs_path)
                    else:
    ...
    def delete(self, metainfos):
        if isinstance(metainfos, FileMeta):
            metainfos = [metainfos]
        
        for metainfo in metainfos:
            op_log.info('del %s' % metainfo.file_id)
            tarfile = self._file_path(metainfo.file_id)
            if os.path.isfile(tarfile):
                op_log.debug('rm %s' % tarfile)
                os.remove(tarfile)
            key = metainfo.primary_key
            FileMeta.mng.remove(metainfo)
    ...
    def _build_meta(cls, src_file):
        ...
        info = {'file_id': '%s%s' % (md5_for_file(src_file), ext),
                'rawname': rawname,
                'sizeInBytes': os.path.getsize(src_file),
                'addtime': time.time(),
                }
        return FileMeta(**info)
